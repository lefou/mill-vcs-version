...
Confusing behaviour in a github action environment, is likely a function of some unintentional or duplicate misuse of `action/checkout`. To check; 

- Ensure the complete repository history is available. (fetch-depth: 0)
- Ensure the tags are available (fetch-tags: true), by defult I believe this should also be set, if the fetch depth is 0.
- That _all_ (not just the first - *cough*) checkout actions in the run, satisfy the two conditions above.
